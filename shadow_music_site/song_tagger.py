...
def _load_tag_pool() -> List[str]:
    config_path = Path(__file__).resolve().parent / "config" / "genre_tags.json"
    with config_path.open("r", encoding="utf-8") as file:
        return json.load(file)


GENRE_TAGS = _load_tag_pool()


class SongTagger:
    def __init__(self, config_store: ConfigStore | None = None, genre_classifier: GenreClassifier | None = None) -> None:
    ...
    def predict(self, song_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "genre_label": str(top_genre.get("tag") or "未知").strip() or "未知",
            "genre_status": "known" if str(top_genre.get("tag") or "") != "未知" else "unknown",
